    ...
    <div className="space-y-6 pt-2">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div>
          ...
          <p className="text-brand-warm-grey mt-2 text-sm">Create and manage assignments</p>
        </div>
        <Button onClick={() => setCreateOpen(true)} className="gap-2 bg-brand-primary hover:bg-brand-primary-dark text-white rounded-xl"><Plus className="h-4 w-4" /> Create Assignment</Button>
      </motion.div>

      <Tabs defaultValue="assignments">
        <TabsList className="bg-brand-cream-dark rounded-xl p-1">
          <TabsTrigger value="assignments" className="rounded-lg data-[state=active]:bg-brand-primary data-[state=active]:text-white text-brand-warm-grey">My Assignments ({assignments.length})</TabsTrigger>
          <TabsTrigger value="submissions" className="rounded-lg data-[state=active]:bg-brand-primary data-[state=active]:text-white text-brand-warm-grey">Submissions {selectedAssignment ? `(${submissions.length})` : ""}</TabsTrigger>
        </TabsList>
          ...
          {isLoading ? (
            ...
            <Card className="bg-white rounded-2xl border border-brand-parchment shadow-[0_2px_24px_rgba(125,30,36,0.06)]">
              <CardContent className="py-12 text-center">
                <div className="w-12 h-12 rounded-full bg-brand-gold-pale flex items-center justify-center mx-auto mb-3"><ClipboardList className="h-6 w-6 text-brand-gold" /></div>
            ...
            <div className="space-y-3">
              {assignments.map((a: any) => (
                <Card key={a.id} className={`cursor-pointer bg-white rounded-2xl border border-brand-parchment shadow-[0_2px_24px_rgba(125,30,36,0.06)] hover:shadow-lg transition-all ${selectedAssignment === a.id ? "ring-2 ring-brand-primary" : ""}`}
                  onClick={() => setSelectedAssignment(selectedAssignment === a.id ? null : a.id)}>
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex items-center gap-3">
                        <div className="p-2 rounded-xl bg-brand-gold-pale"><ClipboardList className="h-5 w-5 text-brand-primary" /></div>
                        <div>
                          <h3 className="font-semibold text-brand-charcoal-mid text-sm">{a.title}</h3>
                          <p className="text-xs text-brand-warm-grey">{a.courses?.title}{a.batches?.name ? ` • ${a.batches.name}` : ""}{a.due_date && ` • Due: ${format(new Date(a.due_date), "MMM dd, yyyy")}`}</p>
                        </div>
                      </div>
          ...
          {!selectedAssignment ? (
            ...
            <div className="space-y-2">
              {submissions.map((sub: any) => (
                <Card key={sub.id} className="bg-white rounded-2xl border border-brand-parchment shadow-[0_2px_24px_rgba(125,30,36,0.06)]">
                  <CardContent className="p-4 flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-brand-charcoal-mid">{sub.student_name}</p>
                      <p className="text-xs text-brand-warm-grey">Submitted: {format(new Date(sub.submitted_at), "MMM dd, yyyy HH:mm")}</p>
                      {sub.text_content && <p className="text-xs mt-1 text-brand-warm-grey line-clamp-2">{sub.text_content}</p>}
                    </div>
                    <div className="flex items-center gap-2">
                      <Badge className={`text-xs border-0 ${sub.status === "graded" ? "bg-green-50 text-green-700" : "bg-amber-50 text-amber-700"}`}>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-w-lg rounded-2xl border-brand-parchment">
          <DialogHeader><DialogTitle className="font-serif text-brand-primary">Create Assignment</DialogTitle></DialogHeader>
          <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
            <div><Label className="text-[11px] uppercase tracking-widest text-brand-warm-grey font-semibold">Title</Label><Input value={form.title} onChange={(e) => updateField("title", e.target.value)} placeholder="Assignment title" className="mt-1 rounded-xl border-brand-parchment focus:border-brand-gold" /></div>
            <div><Label className="text-[11px] uppercase tracking-widest text-brand-warm-grey font-semibold">Description / Instructions</Label><Textarea value={form.description} onChange={(e) => updateField("description", e.target.value)} placeholder="Detailed instructions..." rows={4} className="mt-1 rounded-xl border-brand-parchment focus:border-brand-gold" /></div>
            ...
            <Button onClick={() => createMutation.mutate()} disabled={!form.title || !form.courseId || createMutation.isPending} className="w-full bg-brand-primary hover:bg-brand-primary-dark text-white rounded-xl">
      ...
      <Dialog open={!!feedbackDialog} onOpenChange={() => setFeedbackDialog(null)}>
        <DialogContent className="max-w-sm rounded-2xl border-brand-parchment">
          <DialogHeader><DialogTitle className="font-serif text-brand-primary">Grade & Feedback</DialogTitle></DialogHeader>
          {feedbackDialog && (
            <div className="space-y-4">
              <div><Label className="text-[11px] uppercase tracking-widest text-brand-warm-grey font-semibold">Grade</Label><Input value={feedbackDialog.grade} onChange={(e) => setFeedbackDialog({ ...feedbackDialog, grade: e.target.value })} placeholder="e.g. A+, 95%" className="rounded-xl border-brand-parchment focus:border-brand-gold" /></div>
              <div><Label className="text-[11px] uppercase tracking-widest text-brand-warm-grey font-semibold">Feedback</Label><Textarea value={feedbackDialog.feedback} onChange={(e) => setFeedbackDialog({ ...feedbackDialog, feedback: e.target.value })} placeholder="Your feedback..." className="rounded-xl border-brand-parchment focus:border-brand-gold" /></div>
              <Button onClick={() => gradeMutation.mutate(feedbackDialog)} disabled={gradeMutation.isPending} className="w-full bg-brand-primary hover:bg-brand-primary-dark text-white rounded-xl">
