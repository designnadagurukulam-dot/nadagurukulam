    ...
    <div className="min-h-screen flex bg-brand-cream">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col justify-center items-center p-12 text-center w-full">
          ...
          <p className="text-brand-warm-grey text-center text-sm mt-2 tracking-widest uppercase">
            रसो वै सः
          </p>
      ...
      <div className="flex-1 flex items-center justify-center p-8 md:p-12">
        ...
        >
          ...
          {/* Role tabs */}
          <div className="flex gap-2 mb-6 bg-brand-cream-dark rounded-xl p-1">
            <button
              onClick={() => setTab("student")}
              className={`flex-1 py-3 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 transition-all ${
                tab === "student"
                  ? "bg-brand-primary text-white"
                  : "bg-transparent text-brand-warm-grey hover:bg-brand-parchment"
              }`}
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <Button type="submit" className="w-full h-12 rounded-xl text-base bg-brand-primary hover:bg-brand-primary-dark text-white shadow-sm hover:shadow-md transition-all" disabled={loading}>
              {loading ? "Signing in..." : "Sign In"}
            </Button>
          </form>

          <div className="text-center text-sm text-brand-warm-grey mt-8">
            <Link to={registerLink} className="text-brand-gold hover:text-brand-primary font-semibold underline">
              {registerLabel}
            </Link>
          </div>
          ...
          <p className="text-center text-[11px] text-brand-warm-grey-light mt-8">
            <Link to="/admin-login" className="text-brand-warm-grey-light hover:text-brand-warm-grey underline transition-colors">
