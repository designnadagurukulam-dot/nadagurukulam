    ...
    return (
      <Card key={cls.id} className={`bg-white rounded-2xl border border-brand-parchment shadow-[0_2px_24px_rgba(125,30,36,0.06)] overflow-hidden transition-all hover:shadow-lg ${isPast ? "opacity-70" : ""}`}>
        <CardContent className="p-3.5 sm:p-5">
          ...
          <p className="text-xs sm:text-sm text-brand-warm-grey mt-1">{cls.profiles?.display_name || "Tutor"}</p>
          ...
          <div className="flex items-center gap-2 mt-3 sm:mt-4 flex-wrap">
            ...
            {isPast ? (
              ...
              <>
                <span className="flex items-center gap-1 text-[10px] sm:text-xs text-green-600 font-semibold">
                  <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" /> LIVE
                </span>
                <a href={cls.meeting_link} target="_blank" rel="noopener noreferrer" className="flex-1 sm:flex-none">
                  <Button size="sm" className="w-full sm:w-auto bg-brand-gold text-brand-charcoal hover:bg-brand-gold-light gap-1 rounded-xl text-xs min-h-[40px]">
  ...
  return (
    <div className="space-y-4 sm:space-y-6 pt-2">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-brand-warm-grey mt-1.5 sm:mt-2 text-xs sm:text-sm">Join scheduled live sessions with your tutors</p>
      </motion.div>

      <Tabs defaultValue="upcoming">
        <TabsList className="bg-brand-cream-dark rounded-xl p-1">
          <TabsTrigger value="upcoming" className="rounded-lg data-[state=active]:bg-brand-primary data-[state=active]:text-white text-brand-warm-grey text-xs sm:text-sm min-h-[40px]">Upcoming ({upcoming.length})</TabsTrigger>
          <TabsTrigger value="past" className="rounded-lg data-[state=active]:bg-brand-primary data-[state=active]:text-white text-brand-warm-grey text-xs sm:text-sm min-h-[40px]">Past ({past.length})</TabsTrigger>
        </TabsList>
            ...
            <Card className="bg-white rounded-2xl border border-brand-parchment shadow-[0_2px_24px_rgba(125,30,36,0.06)]">
              <CardContent className="py-10 sm:py-12 text-center">
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-brand-gold-pale flex items-center justify-center mx-auto mb-3">
                  <Video className="h-5 w-5 sm:h-6 sm:w-6 text-brand-gold" />
                </div>
