  ...
  return <div className="space-y-6 pt-2">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      ...
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <Select value={tutor} onValueChange={setTutor}><SelectTrigger><SelectValue placeholder="Tutor" /></SelectTrigger><SelectContent><SelectItem value={ALL}>All tutors</SelectItem>{tutors.map((t: any) => <SelectItem key={t.user_id} value={t.user_id}>{t.display_name || "Tutor"}</SelectItem>)}</SelectContent></Select>
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {filtered.map((item: any) => {
        ...
        return <Card key={item.plan.id} className="border-0 shadow-[0_2px_16px_hsl(var(--primary)/0.06)]"><CardContent className="p-5 space-y-4">
          <div className="flex items-start justify-between gap-3"><div><BookCheck className="h-5 w-5 text-accent mb-2" /><h3 className="font-display text-lg text-brand-primary">{item.module?.subject_name || "Subject"}</h3><p className="text-sm text-muted-foreground">{item.module?.course_code} · {item.teacher?.display_name || "Tutor"}</p></div><Badge variant={item.plan.is_published ? "default" : "secondary"}>{item.plan.is_published ? "Published" : "Draft"}</Badge></div>
    ...
    <Dialog open={!!selected} onOpenChange={(open) => !open && setSelected(null)}><DialogContent className="max-h-[90vh] max-w-5xl overflow-y-auto"><DialogHeader><DialogTitle className="font-display text-brand-primary">{selected?.module?.subject_name} Lesson Plan</DialogTitle></DialogHeader>{selected && <div className="space-y-5">
      <div className="grid gap-3 md:grid-cols-4 text-sm"><div><b>Course</b><p>{selected.module?.course_code}</p></div><div><b>Tutor</b><p>{selected.teacher?.display_name || "—"}</p></div><div><b>Semester</b><p>{selected.plan?.academic_semester || `Semester ${selected.module?.semester}`}</p></div><div><b>Contact Hours</b><p>{selected.plan?.contact_hours_per_week || 3}/week</p></div></div>
